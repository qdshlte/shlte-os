...
/* Preemption flag — set by timer_tick, cleared by schedule */
...
process_t *create_process(void *entry, void *stack, size_t stack_size, const char *name)
{
    ...
    p->exit_code = 0;
    p->ticks = 0;
    p->priority = 128;  /* Default priority */
 ...
 * Called from the idle loop and after timer_tick sets need_reschedule.
 */
void schedule(void)
{
    if (num_processes <= 0) return;

    /* Simple round-robin: scan from the next PID */
    int starting_pid = (current_process ? current_process->pid : 0);
    for (int i = 1; i < MAX_PROCESSES; i++) {
        int next_pid = (starting_pid + i) % MAX_PROCESSES;
        if (process_table[next_pid].pid != 0 &&
            ...
            printk("[SCHED] Switching from pid %d ('%s') to pid %d ('%s')\n",
                   ...
                   process_table[next_pid].pid,
                   process_table[next_pid].name);
            context_switch(current_process, &process_table[next_pid]);
            break;
        }
    }
    need_reschedule = 0;
...
void context_switch(process_t *prev, process_t *next)
{
    ...
    if (prev) {
        save_context(prev);
        prev->state = PROC_RUNNING;  /* Stay runnable for round-robin */
...
void init_process(void)
{
    ...
    /* Register IPC queue for init process */
    ipc_register_queue(init->pid);
