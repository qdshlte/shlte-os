...
/**
 * interrupts_enable - Unmask IRQs in the CPU's PSTATE (DAIF)
 *
 * Clears the IRQ mask bit so that the CPU will receive interrupts.
 */
void interrupts_enable(void)
{
    __asm__ volatile(
        "msr DAIFClr, #0x1\n"   /* Clear IRQ mask (bit 1 of DAIF) */
        :
        :
        : "memory"
    );
...
/**
 * interrupts_disable - Mask IRQs in the CPU's PSTATE (DAIF)
 *
 * Sets the IRQ mask bit so that the CPU will ignore interrupts.
 */
